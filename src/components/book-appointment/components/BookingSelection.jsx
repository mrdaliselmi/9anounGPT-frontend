...
import { ArrowDownIcon, CalendarIcon } from '@radix-ui/react-icons';
import {
  listOfAppointments,
  listOfBusySchedule,
  listOfDays,
...
function BookingSelection({
  ...
  const getListOfAvailableTimes = (selectedDate) => {
    ...
    for (let k = 0; k < selectedProfile.schedule.length; k += 1) {
      if (
        selectedProfile.schedule[k].day === listOfDays[selectedDate.getDay()]
      ) {
    ...
    if (daySchedule) {
      const startingHour = parseInt(
        getHourFromString(get24HrsFrmAMPM(daySchedule.startTime)),
        10,
      );
      const endingHour = parseInt(
        getHourFromString(get24HrsFrmAMPM(daySchedule.endTime)),
        10,
      );
      const interval = 30;

      const listOfAllTimes = createTimeIntervals(
        startingHour,
        endingHour,
        interval,
      );
      // eslint-disable-next-line max-len
      const listOfAvailableTimes = listOfAllTimes.filter(
        (el) => listOfUnavailableTimes.indexOf(el) < 0,
      );
    ...
    if (selectedTime) {
      ...
      return (
        <div className="selection-response response-success">
          <h4>Appointment with {selectedProfile.name}</h4>
          <h5>{timeString}</h5>
          <h5>
            Consultation (30min)
            {' - $'}
